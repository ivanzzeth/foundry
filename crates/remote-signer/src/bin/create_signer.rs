//! Create a new keystore signer via remote-signer API.
//!
//! Env: REMOTE_SIGNER_URL, REMOTE_SIGNER_API_KEY_ID, and one of REMOTE_SIGNER_API_KEY_HEX or REMOTE_SIGNER_API_KEY_FILE,
//!      REMOTE_SIGNER_PASSWORD (password for the new keystore).
...
use remote_signer_client::evm::{CreateKeystoreParams, CreateSignerRequest, CreateSignerResponse};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = CreateSignerRequest {
        signer_type: "keystore".to_string(),
        keystore: Some(CreateKeystoreParams { password }),
    };
    let resp: CreateSignerResponse = client.evm.signers.create(&req)?;
